...
B<Corinna::Schema::List> is a data-oriented object class that reprsents a W3C
B<list>. It is parsed from the W3C schema. Objects of this class contain META
information about the W3C schema B<list> that they represent. It is not used a
building block for the produced B<schema model> however.  Objects of this
...
Returns (GET) and expects (SET) a string value that represents the type of the
items in a 'B<list>' type.  A W3C list type is a whitespace separeted list of
...
Returns (GET) and expects (SET) a Perl Class name that represents the type of
the items in a 'B<list>' type.  A W3C list type is a whitespace separeted list
of tokens (called items) which must have a common atomic type.  This value is
