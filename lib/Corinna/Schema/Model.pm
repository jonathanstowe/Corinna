...
B<Corinna::Schema::Model> is used internally by L<Corinna> for representinng
the parsed information set of a group of W3C schemas.

A B<model> is produced typically by parsing with the
L<Corinna::Schema::Parser/parse()> method. However, it is theoratically
possible to produce it by other means.

A B<model> contains information about all the I<type, element, attribute,
group, and attribute group> definitions that come from the set of schemas that
constitute the I<source> of the model. This includes all global and implicit
types and elements.

Once produced, you can't do much with a model except B<resolve> it. Resolving
the model means things such as resolving all references (such as those
pointing to global elements or groups) and computing the Perl class names that
correspond to each generated class. See L</resolve()> for more details.
...
The B<new()> method will create the I<type>, I<element>, I<attribute>,
I<group>, and I<attributeGroup> fields if it is not passed values for those
fields.
...
A hash of all (global and implicit) type definitions (simple or complex) that
are obtained from the processed W3C schemas.  The hash key is the name of the
B<type> and the value is an object of type L<Corinna::Schema::SimpleType> or
L<Corinna::Schema::ComplexType>, depending on whether this is a simple or
complex type.

A straight forward consequence is that simple and complex types cannot have
name collisions among each other. This conforms with the W3C specifications.

Note that this hash is obtained from a merge of all the information coming
from the various W3C schemas. So it represents information coming from all the
concerned schemas.

Note that each item of this hash later becomes a generated class under the
"Type" subtree when code generation is performed

=head4 element()

A hash of all global elements obtained from the W3C schemas. The hash key is
the name of the global element and the value is an object of type
L<Corinna::Schema::Element>.

Note that this hash is obtained from a merge of all the information coming
from the various W3C schemas. So it represents information coming from all the
concerned schemas.

Note that each item of this hash later becomes a generated class when code
generation is performed.

=head4 attribute()

A hash of all global attributes obtained from the W3C schemas. The hash key is
the name of the global attribute and the value is an object of type
L<Corinna::Schema::Attribute>.

Note that this hash is obtained from a merge of all the information coming
from the various W3C schemas. So it represents information coming from all the
concerned schemas.

Note that no code generation is perfomed for the items in this hash. They are
used internally by the "type" hash once the referenes to them are resolved.

=head4 group()

A hash of all global groups obtained from the W3C schemas. The hash key is the
name of the global group and the value is an object of type
L<Corinna::Schema::Group>.
...
A hash of all global attribute groups obtained from the W3C schemas. The hash
key is the name of the global attribute group and the value is an object of
type L<Corinna::Schema::AttributeGroup>.
...
Add a schema object to the model (to the corresponding hash).  Aliases of
'object' are 'item' and 'node'. So the following are equivalent to the above:

    $model->add(item=>$object);
    $model->add(node=>$object);

In the above, the actual hash where the object will be placed is deduced from
the type of the object.  Possible types are descendents of:
...
One can also pass the name of the hash that one would like the object to be
added. Examples:
...
Normally, when a schema object is already defined within the model, it is an
error to attempt to add it again to the model. This means that the object is
defined twice in the W3C schema. However, this rule is relaxed when the object
within the sceham is marked as I<redefinable> (see
L<Corinna::Schema::Object/is_redefinable()>).  This is typically the case when
we are in a I<redefine> block (when a schema is included wit the redefine
tag).

=head4 xml_item($name, [$nsUri])

Returns the Schema Model item for a given name, and optionally, a namespace
URI. If namespace URI is omitted, then the default namespace URI for the model
is used.

This method does a search on the different hashes kept by the model (element,
type, group, attribute, attributeGroup) in that order, and will return the
first encountred item.

=head4 xml_item_class($name, [$nsUri])

Returns the class name of a Schema Model item for a given name, and
optionally, a namespace URI. If namespace URI is omitted, then the default
namespace URI for the model is used.
...
This method will I<resolve> the B<model>. In other words, thhis method will
prepare the produced model to be processed for code gerenartion.

Resolving a model means: resolving references to global objects (elements and
attributes); replacing group and attributeGroup references with actual
contents of the referenced group; computing the Perl class names of the types
and elements to be generated; and figuring out the inheritance relationships
between classes.

The builtin classes are known to the method so that the Perl classes for them
will not be generated but rather referenced from the L<Corinna::Builtin>
module.
