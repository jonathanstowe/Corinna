...
B<Corinna::Schema::Object> is an B<abstract> ancestor of all L<Corinna> schema
object classes.  Schema object classes are those that are the construction
blocks of a B<schema model> (see L<Corinna::Schema::Model>).  They also
constitute the objects that contain the meta information about a W3C schema
that are embedded as class data within the generated Perl classes by
L<Corinna>.
...
Normally, one does not call the B<new> method on B<Corinna::Schema::Object>.
One rather calls it on the descendant subclasses.
...
This field corresponds to the perl reference (ref) to a resolved W3C reference
to a global object.  It is normally used for elements and attributes. This
way, the local element definition is preserved while having a pointer to the
actual global definition.
...
Returns the B<hash key> that is to be use to hash this object within the
model. This is used by the B<add()> method of the schema model (see
L<Corinna::Schema::Model/add()>). If no B<key> has been previously set, it
returns the B<name> of the object.
...
A schema object can have an ID within the W3C schema. Currently, this property
is not used by L<Corinna>.
...
Normally, when an object is not marked as I<'redefinable'>, it is an error to
attempt to define it again within the schema model. (see
L<Corinna::Schema::Model/add()>).

When processing a schema within a 'redefine' block, all schema objects
...
This field is the B<name> of the schema object as defined within the W3C
schema or as attributed by the parser.  Normally, all global objects (type,
element, group, attribute, attributeGroup) will have a named defined in the
W3C schema.  However, non-global objects (such as local elements) will not
...
will have a named defined in the W3C schema.  However, non-global objects
(such as local elements) will not have a name that comes from the W3C schema.
Such objects will be named by the parser. In this case (when the name is
...
Sometimes in a W3C schema, a local object (such as an element) makes a
reference to a global object (such as a global element). In this case, the
B<'ref'> field will contain the name of the referenced global object.
...
All schema objects are defined within a scope. The scope can be I<'global'> or
...
Sometimes in a W3C schema, an object (such as an attribute or an element) will
refer to a B<type> that is defined globally in the schema set. The value of
...
This method is used to set multiple fields of a schema object all at once.
From an interface point of view, it is like the B<new()> method, but instead
of constructing a new object, it works on an existing object.
...
This is used heavily by the parser in order to set multiple fields that are
obtained by parsing attributes from the schema nodes.
